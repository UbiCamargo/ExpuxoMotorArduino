...
float calibration_factor = 180000.00;                                                        /* Fator de calibração para ajuste da célula */
float peso;  
const int balanca_dt_pin = 3;
const int balanca_sck_pin = 2;                                                             /* variável peso */

void setup() {                                                                              /* rotina de configurações */
  balanca.begin(balanca_dt_pin, balanca_sck_pin);
...
void loop() {                                                                               /* chama função de loop */
  ...
  delay(300);                                                                               /* atraso de 500ms = 0.5s*/

  if(Serial.available())                                                                    /* caso sejam inseridos caracteres no serial */
  {
    char temp = Serial.read();
    if(temp == '+')                                                                         /* Se o + for pressionado     */
      calibration_factor += 100;                                                              /* incrementa 1 no fator de calibração   */
    else if(temp == '-')                                                                    /* Caso o - seja pressionado   */
      calibration_factor -= 100;                                                              /* Decrementa 1 do fator de calibração  */
  }
}